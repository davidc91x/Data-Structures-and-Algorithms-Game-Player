 ...
 */
#define PLAYER_NAME LOLITOIOhoa1
        ...
        if(c.id!=-1  and unit(c.id).player!=me() and not demonio16(v)) {
          ...
          for(int i=0;i<=7;++i) {
           Pos v1=v+Dir(orden[i]);
            if(pos_ok(v1) and dist[v1.i][v1.j]==INFINIT and (casillas[orden[i]] or ( ( cell(v1).id==-1 or unit(cell(v1).id).player!=me() ) and not demonio16(v1)))){
                    S.push(v1);
