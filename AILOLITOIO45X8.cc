 ...
 */
#define PLAYER_NAME LOLITOIO45X8
    ...
    while(not S.empty()){
        ...
        if(c.id!=-1  and unit(c.id).player!=me() and unit(c.id).type==Pioneer and not demonio16(v)) {
          ...
          if(v2!=o)  di=pre[v2.i][v2.j];
            ...
            for(int i=0;i<=7;++i) {
           Pos v1=v+Dir(i);
            if(pos_ok(v1) and dist[v1.i][v1.j]==INFINIT and (casillas[i] or ( ( cell(v1).id==-1 or( unit(cell(v1).id).player!=me() and unit(cell(v1).id).type==Pioneer)) and not demonio16(v1)))){ //meter not enemy?
                     S.push(v1);
                    dist[v1.i][v1.j]=dist[v.i][v.j]+1;
                      pre[v1.i][v1.j]=Dir(i);
    ...
    while(not S.empty()){
        ...
        if(c.id!=-1  and unit(c.id).player!=me() and unit(c.id).type==Furyan and not demonio16(v)) {
          ...
          if(v2!=o)  di=pre[v2.i][v2.j];
           ...
           for(int i=0;i<=7;++i) {
           Pos v1=v+Dir(i);
            if(pos_ok(v1) and dist[v1.i][v1.j]==INFINIT and (casillas[i] or ( ( cell(v1).id==-1 or( unit(cell(v1).id).player!=me() and unit(cell(v1).id).type==Furyan)) and not demonio16(v1)))){
                     S.push(v1);
                    dist[v1.i][v1.j]=dist[v.i][v.j]+1;
                      pre[v1.i][v1.j]=Dir(i);
