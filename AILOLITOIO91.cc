          ...
          if(ene.player!=-1 and ene.player!=me() and ene.type==Furyan ){ //añadir caso de si no es pared ir al menos parecido posible??
            for(int f=0;f<=7;++f) {
              Dir dc=Dir(cas[f]);
               Cell cc=cell(pp+dc);
               int idc=cc.id;
              if(dc!=dp and cc.type==Cave and cc.owner==rival and not enemy(pp+dc)) return  dc;
            }
            for(int f=0;f<=7;++f) {
              Dir dc=Dir(cas[f]);
               Cell cc=cell(pp+dc);
               int idc=cc.id;
              if(dc!=dp and cc.type==Cave and cc.owner!=me() and not enemy(pp+dc)) return  dc;
            }
            for(int f=0;f<=7;++f) {
              Dir dc=Dir(cas[f]);
               Cell cc=cell(pp+dc);
               int idc=cc.id;
              if(dc!=dp and cc.type!=Rock and not enemy(pp+dc) ) return  dc;
            }
            for(int f=0;f<=7;++f) {
              Dir dc=Dir(cas[f]);
               Cell cc=cell(pp+dc);
