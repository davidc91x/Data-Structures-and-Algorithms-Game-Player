 ...
 Pos bfsataquecuevarival(Pos o, int &d, Dir &di){ //algoritmo de busqueda de enemigos de otros equipos en subsuelo evitando pasar por casillas que a su alrededor tienen a un monstruo y evivtando pisar a unidades en el primer paso para llegar al enemigo.
   d=-1;
    ...
    while(not S.empty()){
        ...
        S.pop();
        Cell c=cell(v);
        if(c.id!=-1  and unit(c.id).player==rival and not demonio(v)) {
          d=dist[v.i][v.j];
          di=pre[v.i][v.j];
          Pos v2=v;
          while(v2!=o) {
   ...
   //   Dir dih=huir(pp);
    ...
    for(int b=0;b<=7;++b) {
      ...
      if(b!=pp) {
        ...
        if(b3!=pp and (dist==-1 or disss<=dist)) priocomandos.emplace(make_pair(4,make_pair(disss,0)),make_pair(Pi[P[a]],diss));
        if(b5!=pp and (dist==-1 or dist5<=dist)) priocomandos.emplace(make_pair(4,make_pair(dist5,0)),make_pair(Pi[P[a]],di5));
...
/*
    ...
    void cargarataques(){
     ...
     //   Pos b3=bfsataquecuevarival2(pf,dist3,di3);
        if(pf!=b2 and (dist==-1 or dist2<=dist)) {
