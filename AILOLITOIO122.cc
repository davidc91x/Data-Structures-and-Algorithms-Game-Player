 ...
 */
#define PLAYER_NAME LOLITOIO122
...
return None;
   ...
   //     else if(pp==c) priocomandos.emplace(make_pair(0,make_pair(1,-2600+u.health)),make_pair(Pi[P[a]],Up));
    ...
    for(int b=0;b<=7;++b) {
        ...
        if(idp!=-1 ) {
          Unit ene= unit(idp);
          if(ene.player!=-1 and ene.player!=me() and ene.type==Furyan){ //METER NOT DEMONIO16 PARA HUIR BIEN
            for(int f=0;f<=7;++f) {
              Dir dc=Dir(cas[f]);
               Cell cc=cell(pp+dc);
               int idc=cc.id;
              if(dc!=dp and cc.type==Cave and cc.owner==rival and not enemy(pp+dc) and not demonio16(pp+dc)) priocomandos.emplace(make_pair(0,make_pair(1,(-2600+u.health))),make_pair(Pi[P[a]],dc));
            }
            for(int f=0;f<=7;++f) {
              Dir dc=Dir(cas[f]);
               Cell cc=cell(pp+dc);
               int idc=cc.id;
              if(dc!=dp and cc.type==Cave and cc.owner==rival2 and not enemy(pp+dc) and not demonio16(pp+dc)) priocomandos.emplace(make_pair(0,make_pair(1,(-2600+u.health))),make_pair(Pi[P[a]],dc));
            }
            for(int f=0;f<=7;++f) {
              Dir dc=Dir(cas[f]);
               Cell cc=cell(pp+dc);
               int idc=cc.id;
              if(dc!=dp and cc.type==Cave and cc.owner==rival3 and not enemy(pp+dc) and not demonio16(pp+dc)) priocomandos.emplace(make_pair(0,make_pair(1,(-2600+u.health))),make_pair(Pi[P[a]],dc));
            }
            for(int f=0;f<=7;++f) {
              Dir dc=Dir(cas[f]);
               Cell cc=cell(pp+dc);
               int idc=cc.id;
              if(dc!=dp and cc.type==Cave and cc.owner!=me() and not enemy(pp+dc) and not demonio16(pp+dc)) priocomandos.emplace(make_pair(0,make_pair(1,(-2600+u.health))),make_pair(Pi[P[a]],dc));
            }
            ...
            for(int f=0;f<=7;++f) {
              Dir dc=Dir(cas[f]);
               Cell cc=cell(pp+dc);
               int idc=cc.id;
              if(dc!=dp and cc.type!=Rock and not enemy(pp+dc) and not demonio16(pp+dc) ) priocomandos.emplace(make_pair(0,make_pair(1,(-2600+u.health))),make_pair(Pi[P[a]],dc));
            }
            for(int f=0;f<=7;++f) {
              Dir dc=Dir(cas[f]);
               Cell cc=cell(pp+dc);
               int idc=cc.id;
              if(dc!=dp and cc.type==Cave and cc.owner==rival and cc.id==-1 and not demonio16(pp+dc)) priocomandos.emplace(make_pair(0,make_pair(1,(-2600+u.health))),make_pair(Pi[P[a]],dc));
            }
            for(int f=0;f<=7;++f) {
              Dir dc=Dir(cas[f]);
               Cell cc=cell(pp+dc);
               int idc=cc.id;
              if(dc!=dp and cc.type==Cave and cc.owner==rival2 and cc.id==-1 and not demonio16(pp+dc)) priocomandos.emplace(make_pair(0,make_pair(1,(-2600+u.health))),make_pair(Pi[P[a]],dc));
            }
            for(int f=0;f<=7;++f) {
              Dir dc=Dir(cas[f]);
               Cell cc=cell(pp+dc);
               int idc=cc.id;
              if(dc!=dp and cc.type==Cave and cc.owner==rival3 and cc.id==-1 and not demonio16(pp+dc)) priocomandos.emplace(make_pair(0,make_pair(1,(-2600+u.health))),make_pair(Pi[P[a]],dc));
            }
            for(int f=0;f<=7;++f) {
              Dir dc=Dir(cas[f]);
               Cell cc=cell(pp+dc);
               int idc=cc.id;
              if(dc!=dp and cc.type==Cave and cc.owner!=me() and cc.id==-1 and not demonio16(pp+dc)) priocomandos.emplace(make_pair(0,make_pair(1,(-2600+u.health))),make_pair(Pi[P[a]],dc));
            }
            ...
            for(int f=0;f<=7;++f) {
              Dir dc=Dir(cas[f]);
               Cell cc=cell(pp+dc);
               int idc=cc.id;
              if(dc!=dp and cc.type!=Rock and cc.id==-1 and not demonio16(pp+dc)) priocomandos.emplace(make_pair(0,make_pair(1,(-2600+u.health))),make_pair(Pi[P[a]],dc));
            }
            ...
            for(int f=0;f<=7;++f) {
              Dir dc=Dir(cas[f]);
               Cell cc=cell(pp+dc);
               int idc=cc.id;
              if(dc!=dp and cc.type==Cave and cc.owner==rival and cc.id==-1 and not demonio(pp+dc)) priocomandos.emplace(make_pair(0,make_pair(1,(-2600+u.health))),make_pair(Pi[P[a]],dc));
            }
            for(int f=0;f<=7;++f) {
              Dir dc=Dir(cas[f]);
               Cell cc=cell(pp+dc);
               int idc=cc.id;
              if(dc!=dp and cc.type==Cave and cc.owner==rival2 and cc.id==-1 and not demonio(pp+dc)) priocomandos.emplace(make_pair(0,make_pair(1,(-2600+u.health))),make_pair(Pi[P[a]],dc));
            }
            for(int f=0;f<=7;++f) {
              Dir dc=Dir(cas[f]);
               Cell cc=cell(pp+dc);
               int idc=cc.id;
              if(dc!=dp and cc.type==Cave and cc.owner==rival3 and cc.id==-1 and not demonio(pp+dc)) priocomandos.emplace(make_pair(0,make_pair(1,(-2600+u.health))),make_pair(Pi[P[a]],dc));
            }
            for(int f=0;f<=7;++f) {
              Dir dc=Dir(cas[f]);
               Cell cc=cell(pp+dc);
               int idc=cc.id;
              if(dc!=dp and cc.type==Cave and cc.owner!=me() and cc.id==-1 and not demonio(pp+dc)) priocomandos.emplace(make_pair(0,make_pair(1,(-2600+u.health))),make_pair(Pi[P[a]],dc));
            }
            for(int f=0;f<=7;++f) {
              Dir dc=Dir(cas[f]);
               Cell cc=cell(pp+dc);
            ...
            for(int f=0;f<=7;++f) {
              Dir dc=Dir(cas[f]);
               Cell cc=cell(pp+dc);
               int idc=cc.id;
              if(dc!=dp and cc.type!=Rock and cc.id==-1 and not demonio(pp+dc)) priocomandos.emplace(make_pair(0,make_pair(1,(-2600+u.health))),make_pair(Pi[P[a]],dc));
            }
            for(int f=0;f<=7;++f) {
              Dir dc=Dir(cas[f]);
               Cell cc=cell(pp+dc);
               int idc=cc.id;
              if(dc!=dp and cc.type==Cave and cc.owner==rival and cc.id==-1 ) priocomandos.emplace(make_pair(0,make_pair(1,(-2600+u.health))),make_pair(Pi[P[a]],dc));
            }
            for(int f=0;f<=7;++f) {
              Dir dc=Dir(cas[f]);
               Cell cc=cell(pp+dc);
               int idc=cc.id;
              if(dc!=dp and cc.type==Cave and cc.owner==rival2 and cc.id==-1 ) priocomandos.emplace(make_pair(0,make_pair(1,(-2600+u.health))),make_pair(Pi[P[a]],dc));
            }
            for(int f=0;f<=7;++f) {
              Dir dc=Dir(cas[f]);
               Cell cc=cell(pp+dc);
               int idc=cc.id;
              if(dc!=dp and cc.type==Cave and cc.owner==rival3 and cc.id==-1 ) priocomandos.emplace(make_pair(0,make_pair(1,(-2600+u.health))),make_pair(Pi[P[a]],dc));
            }
            for(int f=0;f<=7;++f) {
              Dir dc=Dir(cas[f]);
               Cell cc=cell(pp+dc);
               int idc=cc.id;
              if(dc!=dp and cc.type==Cave and cc.owner!=me() and cc.id==-1 ) priocomandos.emplace(make_pair(0,make_pair(1,(-2600+u.health))),make_pair(Pi[P[a]],dc));
            }
            for(int f=0;f<=7;++f) {
              Dir dc=Dir(cas[f]);
               Cell cc=cell(pp+dc);
            ...
            for(int f=0;f<=7;++f) {
              Dir dc=Dir(cas[f]);
               Cell cc=cell(pp+dc);
               int idc=cc.id;
              if(dc!=dp and cc.type!=Rock and cc.id==-1 ) priocomandos.emplace(make_pair(0,make_pair(1,(-2600+u.health))),make_pair(Pi[P[a]],dc));
            }
            for(int f=0;f<=7;++f) {
              Dir dc=Dir(cas[f]);
               Cell cc=cell(pp+dc);
               int idc=cc.id;
              if(dc!=dp and cc.type!=Rock and idc!=-1 and unit(idc).type==Pioneer) priocomandos.emplace(make_pair(0,make_pair(1,(-2600+u.health))),make_pair(Pi[P[a]],dc));
            }
            for(int f=0;f<=7;++f) {
              Dir dc=Dir(cas[f]);
               Cell cc=cell(pp+dc);
               int idc=cc.id;
              if(dc!=dp and cc.type!=Rock) priocomandos.emplace(make_pair(0,make_pair(1,(-2600+u.health))),make_pair(Pi[P[a]],dc));
            }
